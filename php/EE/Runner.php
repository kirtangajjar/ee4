...
class Runner {
	...
	public function load_wordpress() {
		...
		if ( ! empty( $ee_is_loaded ) ) {
			return;
		}

		$ee_is_loaded = true;

		EE::debug( 'Begin WordPress load', 'bootstrap' );
		EE::do_hook( 'before_wp_load' );
		...
		EE::debug( 'wp-config.php path: ' . $wp_config_path, 'bootstrap' );
		EE::do_hook( 'before_wp_config_load' );

		// Load wp-config.php code, in the global scope
		$ee_original_defined_vars = get_defined_vars();
		eval( $this->get_wp_config_code() );
		foreach ( get_defined_vars() as $key => $var ) {
			if ( array_key_exists( $key, $ee_original_defined_vars ) || 'ee_original_defined_vars' === $key ) {
				continue;
			}
			global ${$key};
			${$key} = $var;
		...
		if ( Utils\wp_version_compare( '4.6-alpha-37575', '>=' ) ) {
			...
			require EE_ROOT . '/php/wp-settings-cli.php';
		}
		...
		EE::debug( 'Loaded WordPress', 'bootstrap' );
		EE::do_hook( 'after_wp_load' );
	...
	private function setup_bootstrap_hooks() {
		...
		if ( $this->config['skip-themes'] ) {
			EE::add_wp_hook( 'setup_theme', array( $this, 'action_setup_theme_ee_skip_themes' ), 999 );
		}
		...
		if ( $this->cmd_starts_with( array( 'help' ) ) ) {
			...
			EE::add_wp_hook(
				'wp_die_handler',
				function() {
					return '\EE\Utils\wp_die_handler';
				}
			);
		}

		// Prevent code from performing a redirect
		EE::add_wp_hook( 'wp_redirect', 'EE\\Utils\\wp_redirect_handler' );
		...
		EE::add_wp_hook( 'ms_site_check', '__return_true' );
		...
		// Never load advanced-cache.php drop-in when EE is operating
		EE::add_wp_hook(
		...
		if ( $this->is_multisite() ) {
			...
			EE::add_wp_hook(
				...
				function( $current_site, $domain, $path ) {
					...
					$has_param = isset( EE::get_runner()->config['url'] );
		...
		EE::add_wp_hook(
			...
			function() {
				if ( $GLOBALS['_wp_using_ext_object_cache'] && class_exists( 'APC_Object_Cache' ) ) {
					EE::warning( 'Running EE while the APC object cache is activated can result in cache corruption.' );
					EE::confirm( 'Given the consequences, do you wish to continue?' );
				}
			},
			0
		);
	...
	private function setup_skip_plugins_filters() {
		$ee_filter_active_plugins = function( $plugins ) {
			$skipped_plugins = EE::get_runner()->config['skip-plugins'];
		...
		foreach ( $hooks as $hook ) {
			EE::add_wp_hook( $hook, $ee_filter_active_plugins, 999 );
		}
		EE::add_wp_hook(
			'plugins_loaded',
			function() use ( $hooks, $ee_filter_active_plugins ) {
				foreach ( $hooks as $hook ) {
					remove_filter( $hook, $ee_filter_active_plugins, 999 );
				}
			},
			0
		);
	 ...
	 */
	public function action_setup_theme_ee_skip_themes() {
		$ee_filter_active_theme = function( $value ) {
			$skipped_themes = EE::get_runner()->config['skip-themes'];
		...
		foreach ( $hooks as $hook ) {
			add_filter( $hook, $ee_filter_active_theme, 999 );
		}
		// Clean up after the TEMPLATEPATH and STYLESHEETPATH constants are defined
		EE::add_wp_hook(
			'after_setup_theme',
			function() use ( $hooks, $ee_filter_active_theme ) {
				foreach ( $hooks as $hook ) {
					remove_filter( $hook, $ee_filter_active_theme, 999 );
				}
			},
			0
		);
