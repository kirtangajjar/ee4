...
class UtilsTest extends PHPUnit_Framework_TestCase {
	...
	public function testGetTempDir() {
		...
		if ( version_compare( PHP_VERSION, '5.5.0', '>=' ) ) {
			...
			$cmd = 'php ' . escapeshellarg( '-dsys_temp_dir=\\tmp\\' ) . ' php/boot-fs.php --skip-wordpress eval ' . escapeshellarg( 'echo EE\\Utils\\get_temp_dir();' ) . ' 2>&1';
			$output = array();
			exec( $cmd, $output );
			$output = trim( implode( "\n", $output ) );
			...
			$cmd = 'php ' . escapeshellarg( '-dsys_temp_dir=' ) . ' php/boot-fs.php --skip-wordpress eval ' . escapeshellarg( 'echo EE\\Utils\\get_temp_dir();' ) . ' 2>&1';
			$output = array();
			exec( $cmd, $output );
			$output = trim( implode( "\n", $output ) );
	...
	public function testRunMysqlCommandProcDisabled() {
		...
		$cmd = 'php -ddisable_functions=proc_open php/boot-fs.php --skip-wordpress eval ' . escapeshellarg( 'EE\\Utils\\run_mysql_command( null, array() );' ) . ' 2>&1';
		$output = array();
		exec( $cmd, $output );
		$output = trim( implode( "\n", $output ) );
		$this->assertTrue( false !== strpos( $output, $err_msg ) );

		$cmd = 'php -ddisable_functions=proc_close php/boot-fs.php --skip-wordpress eval ' . escapeshellarg( 'EE\\Utils\\run_mysql_command( null, array() );' ) . ' 2>&1';
		$output = array();
		exec( $cmd, $output );
		$output = trim( implode( "\n", $output ) );
		$this->assertTrue( false !== strpos( $output, $err_msg ) );
	...
	public function testLaunchEditorForInputProcDisabled() {
		...
		$cmd = 'php -ddisable_functions=proc_open php/boot-fs.php --skip-wordpress eval ' . escapeshellarg( 'EE\\Utils\\launch_editor_for_input( null, null );' ) . ' 2>&1';
		$output = array();
		exec( $cmd, $output );
		$output = trim( implode( "\n", $output ) );
		$this->assertTrue( false !== strpos( $output, $err_msg ) );

		$cmd = 'php -ddisable_functions=proc_close php/boot-fs.php --skip-wordpress eval ' . escapeshellarg( 'EE\\Utils\\launch_editor_for_input( null, null );' ) . ' 2>&1';
		$output = array();
		exec( $cmd, $output );
		$output = trim( implode( "\n", $output ) );
		$this->assertTrue( false !== strpos( $output, $err_msg ) );
